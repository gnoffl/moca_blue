...
DATE= "20240213"
#######################################################
FILE1= "Atha-0h_scores.h5"
FILE2= "Atha-0h_model_genes_and_pred_prob.csv"
FILE3= "He-etal2021_Atha_0h_logmaxTPM.2.csv"

#######################################################
dirpath_in1 = "~/Desktop/Rhome/moca_blue/0MOTIFS/He_etal_Models/"
dirpath_in2 = "~/Desktop/Rhome/kallisto_handling/tpm_classification_for_deepCRE/"
dirpath_out = "~/Desktop/Rhome/moca_blue/0MOTIFS/"
# # # # # # # # # # # # # # # # # # # # # # # # # # # #
file_path1 = file.path(dirpath_in1,FILE1)
file_path2 = file.path(dirpath_in2,FILE3)
# # # # # # # # # # # # # # # # # # # # # # # # # # # #
#####################################################
#######################################################
...
saliency_scores <- h5read(h5file,
                          "contrib_scores")
########### ################ ################ #########
pickle_list <- py_load_object("genes.3.pickle")
# pickle_list <- readRDS("validation_genes.pickle")
######################################################
model_parameter <- read.table(
  ...
  sep=",")
#######################################################
colnames(model_parameter)[1] <- "gene_id"
colnames(model_parameter)[2] <- "pred_prob"
model_parameter$pred_class <- ifelse(model_parameter$pred_prob > 0.501, 1, 0)

# # # # # # # # # # # # # # # # # # # # # # # # # # # #
model_tpm <- read.table(
  file.path(
    dirpath_in2,
    FILE3),
  header=TRUE,
...
########################################################
colnames(model_tpm)[1] <- "gene_id"
lower_quantile <- quantile(model_tpm$logMaxTPM, 0.25)
upper_quantile <- quantile(model_tpm$logMaxTPM, 0.75)
model_tpm$true_target <- ifelse(model_tpm$logMaxTPM <= lower_quantile, 0,
                                 ifelse(model_tpm$logMaxTPM > upper_quantile, 1, 2))
model_tpm <- model_tpm %>%
  filter(!grepl("^ATM|^ATC", gene_id))
table(model_tpm$true_target)
model_tpm0 <- select(model_tpm, gene_id, logMaxTPM, true_target)
model_parameter0 <- merge(model_parameter, model_tpm0, by= "gene_id")
model_parameter1 <- model_parameter0 %>%
  filter(true_target !="2")
model_parameter1$acc_value <- ifelse(model_parameter1$pred_class ==
                                       model_parameter1$true_target, 1, 0) 
table(model_parameter1$acc_value)

#######################################################
####################################################### # accuracy with prediction too low
...
#model_parameter$TRUE_class <- ifelse(model_parameter$pred_class == 
#                                       model_parameter$true_target, TRUE, FALSE)

############ ##############################
#model_parameter_0 <- model_parameter[, c(1,2,4,7,9)] # select values of interest
model_parameter_0 <- model_parameter[, c(1,2)]

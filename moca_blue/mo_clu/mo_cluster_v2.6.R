...
NAME0="rdf5_2epm"
SPEC ="At"
MODEL="S0S1" # C0 stand for DeepCistrome version 1 (available at 02-may-2023) Standard conditions
TYPE ="_cwm-motifs.jaspar"
#######################################################
#FILE1 = paste0(NAME0,SPEC,MODEL,TYPE)
FILE1 = "rdf5_2epmArthS0+S1_cwm-motifs.jaspar"
#######################################################
...
lower_percentile <- max(c1)*0.05
#upper_percentile <- quantile(c1, probs = 0.95)
upper_percentile <- max(c1)*0.90
###
string_to_remove1 <- "S0_"
string_to_remove2 <- "S1_"

...
###########################################################################################
specific_genes <- solanum_df[solanum_df$specific == "specific", ]
non_specific_genes <- solanum_df[solanum_df$specific == "non-specific", ]
...
# Melt the dataframe to long format
melted_results <- gather(results, key = "variable", value = "accuracy", -iteration)

# Convert accuracy to percentage
melted_results$accuracy <- melted_results$accuracy * 100

# Plotting
p <- ggplot(melted_results, aes(x = variable, y = accuracy)) +
  geom_jitter(color = "darkgrey", shape = 4, width = 0.1) +
  geom_boxplot(fill = "transparent", color = "black") +
  geom_hline(yintercept = c(25, 50, 75), linetype = "dashed", color = "grey") +
  geom_hline(yintercept = c(0), color = "grey") +
  labs(x = "Variable", y = "Accuracy (%)") +
  scale_y_continuous(breaks = seq(0, 100, by = 25), limits = c(0, 100)) +  
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
        panel.background = element_rect(fill = "transparent"),
        plot.background = element_rect(fill = "transparent"))
p
ggsave("bts100_solanum_cross_performance.png", plot = p,
       width = 12,
       height = 12, 
       units = "in", dpi = 300)
